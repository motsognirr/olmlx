...
- **Flash inference** (`engine/flash/`): LLM in a Flash — sparse FFN with SSD-backed neuron loading for running models larger than GPU memory. One-time prep: `olmlx flash prepare <model>` bundles FFN weights into per-layer `.flashweights` files, trains low-rank sparsity predictors, and writes a `flash_layout.json` next to the model. Runtime: `FlashModelWrapper` replaces FFN layers with `FlashMLP` instances that consult the predictor for each token, load active neurons from SSD via `FlashWeightStore` (`LayerLruCache` + optional preallocated buffer + optional `O_DIRECT`/`F_NOCACHE`), then run the standard SwiGLU path on the resident neurons. Primary knobs were promoted out of `experimental` in PR #274: `OLMLX_FLASH`, `OLMLX_FLASH_SPARSITY_THRESHOLD`, `OLMLX_FLASH_MIN_ACTIVE_NEURONS`, `OLMLX_FLASH_MAX_ACTIVE_NEURONS`, `OLMLX_FLASH_MEMORY_BUDGET_FRACTION` (also `--flash` on `olmlx serve`). Per-model overrides for these five fields go at the top level of `models.json`. Advanced tuning (window size, IO threads, cache budget, predictor rank, bypass-OS-cache, preallocated buffer, prefetch, flash-speculative, flash-MoE) remains under `OLMLX_EXPERIMENTAL_FLASH_*` and `models.json` `experimental.flash_*`. Legacy `OLMLX_EXPERIMENTAL_FLASH*` for the five promoted fields is honoured for one release with a deprecation warning; per-model `experimental` blocks containing the promoted keys raise a migration error at startup. The shim (`surface_legacy_flash_env`) lives in `olmlx.config` rather than `olmlx.cli` so `distributed_worker.worker_main` can reuse it without pulling in argparse/uvicorn. Validated on Qwen3-family text models (`Qwen3`, `Qwen3-Coder`, `Qwen3.5`/`Qwen3-Coder-Next` text towers). MoE targets use a separate Flash-MoE code path. The bench runner exposes `flash` / `flash+tq4` scenarios for tokens/sec + RAM measurements. **Distributed caveat**: per-model Flash configuration in `models.json` (top-level `flash` plus the four numeric knobs) is honoured only in single-node mode. `_launch_distributed_workers` forwards `settings.flash*` (global) to workers and `worker_main` reads the same globals — a per-model override is silently ignored on workers because the registry isn't consulted there. Pre-existing limitation, but worth noting since promotion makes per-model `flash: true` look like a first-class option.
...
- **Distributed inference**: Splits models across multiple Apple Silicon machines using MLX's ring distributed backend. Startup sequence: CLI generates ring hostfile → launches workers via SSH → ring init (`mx.distributed.init`) → starts sideband server (TCP, port 32400) → starts uvicorn. Key constraints:
