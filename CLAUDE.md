...
│   │   ├── prepare.py       # `olmlx dflash prepare` training pipeline (CE on masked positions)
...
- **Speculative decoding** (`engine/speculative.py`, `engine/dflash/`): Two strategies share the same `prefill`/`step`/`reset` protocol and `verify_draft_greedy` primitive, dispatched by `OLMLX_SPECULATIVE_STRATEGY`:
