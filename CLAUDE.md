...
- **Message-boundary checkpoint path** (prompt cache): For non-trimmable cache layouts — `ArraysCache` SSM hybrids (Qwen3.5, Nemotron-H, Jamba) and mixed Rotating+Arrays (Qwen3-Next) — prefill runs in **at most two chunks** with one snapshot at the deepest interior message boundary. `snapshot_cache_for_persistence` does deepcopy + eager `mx.eval` to materialize lazy `gated_delta_kernel` graphs before the cross-thread crossing (fixes the Metal-stream crash, #284, #343, #396). Snapshotting at every boundary compounds GatedDeltaNet kernel non-chunking-invariance — invisible on dense full-precision targets but derails MoE-quantized targets (Qwen3.6-35B-A3B-6bit) into verbatim repetition by turn 2. Two chunks bounds the drift to a constant.
- **Pure-`RotatingKVCache` (SWA) prefill is single-chunk**: sliding-window models with no `ArraysCache` layers (gpt-oss, Step-3.5, Gemma 3 — detected by `_is_pure_rotating_cache`) are prefilled in **one** `model()` call, with one snapshot at the end (KV depth `len-1`). The two-chunk split above corrupts their windowed attention — splitting prefill at an interior message boundary makes them emit coherent-but-unrelated text and skip tool calls (only with `tools`, since tool defs add the `system`/`developer` segments that create the boundary). The two-chunk split exists solely to bound GatedDeltaNet drift, which these models don't have.
