...
- **EAGLE draft training** (`olmlx eagle prepare`): Train an EAGLE draft locally for any target. Phase D supports only `--use-precomputed` mode — the operator runs `olmlx dflash precompute` first to dump target hiddens, then `olmlx eagle prepare <target> --use-precomputed <shard-dir>` consumes the *deepest* captured layer (slicing `hidden_concat[:, :, -hidden_size:]` per batch — the same shards work for both DFlash and EAGLE). Loss is autoregressive next-token CE under teacher forcing: at each position `t`, draft input is `(input_ids[t], target_hidden[t])` and label is `input_ids[t+1]`. AdamW + cosine LR schedule. Output: `<model-dir>/eagle/{config.json, model-00001-of-00001.safetensors}` with a top-level `eagle_config.{block_size}` block so `_load_eagle_decoder` can distinguish from DFlash drafts at load time. The borrowed `embed_tokens` / `lm_head` are explicitly excluded from the saved checkpoint via `object.__setattr__` in `bind()` (keeps them out of the parameter tree to avoid duplicating ~250M target parameters and to prevent `nn.value_and_grad` from training the target's frozen weights).
