...
- **Speculative decoding** (`OLMLX_SPECULATIVE_STRATEGY`): Four strategies share `prefill`/`step`/`reset` + `verify_draft_greedy`.
  ...
  - **Prefill cancellation**: `prefill()` checks `cancel_event` at each `_PREFILL_CHUNK` boundary and raises `PrefillCancelled` (caught in `speculative_stream_generate` → clean stream exit, no token). A client disconnect mid-prefill (e.g. a ~69k-token agentic prompt) now releases the GPU + inference lock within one sub-chunk instead of running prefill to completion and 503-ing the next request via deferred cleanup. `classic`/`pld` honor it (shared `_chunked_prefill`/`_prefill_last_logit`); `dflash`/`eagle` accept the kwarg but prefill in a single forward, so it's a no-op for them.
