...
async def main() -> int:
    ...
    print(f"prompt_cache={'ON' if cache_on else 'OFF'}  rounds={args.rounds}  "
          f"gens/load={args.gens_per_load}  models={len(models)}")
    print(f"{'round':>5} {'metal_floor_GiB':>16} {'rss_floor_GiB':>14} "
    ...
    for r in range(1, args.rounds + 1):
        ...
        print(f"{r:>5} {_gib(metal):>16.3f} {_gib(rss):>14.3f} "
    ...
    if len(floors) >= 3:
        ...
        print(f"warmup jump round1->2 RSS: "
              f"{(floors[1][1] - floors[0][1]) / 1024**2:+.1f} MiB (one-time)")
        print(f"metal floor growth: {grow_metal:+.1f} MiB over {n_steady} steady rounds")
        print(f"rss   floor growth: {grow_rss:+.1f} MiB over {n_steady} steady rounds "
