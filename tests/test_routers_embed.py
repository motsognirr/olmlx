...
class TestEmbedRouter:
    ...
    async def test_embed_string(self, app_client):
        with patch("mlx_ollama.routers.embed.generate_embeddings", new_callable=AsyncMock) as mock_emb:
            mock_emb.return_value = [[0.1, 0.2, 0.3]]
            resp = await app_client.post("/api/embed", json={
                "model": "qwen3",
                "input": "hello",
            })
    ...
    async def test_embeddings_legacy(self, app_client):
        with patch("mlx_ollama.routers.embed.generate_embeddings", new_callable=AsyncMock) as mock_emb:
            ...
            resp = await app_client.post("/api/embeddings", json={
                "model": "qwen3",
                "prompt": "hello",
            })
