"""Live STT+TTS round-trip (real models; deselected in CI via -m "not real_model").
...
Requires the [voice] extra (mlx-audio) and ffmpeg on PATH. Run locally with
`uv run pytest tests/live/test_voice_real.py`.
"""
