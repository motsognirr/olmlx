...
class OlmlxStatsCollector:
    ...
    def collect(self):
        ...
        batch_fairness_c = CounterMetricFamily(
            "olmlx_batch_fairness_pauses_total",
            "Times the batch yielded the inference lock to a waiting "
            "exclusive request (fairness pause latched).",
            labels=["model"],
        )
        ...
        def _emit_one(lm: object) -> None:
            ...
            if sched is not None and hasattr(sched, "stats"):
                ...
                batch_fairness_c.add_metric(
                    [name],
                    self._acc.total(
                        f"{name}|b_fair", bs.get("batch_fairness_pauses", 0)
                    ),
                )
