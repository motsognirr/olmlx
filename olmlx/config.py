...
class Settings(BaseSettings):
    ...
    image_max_prompt_chars: Annotated[
        int,
        Field(
            gt=0,
            description="Max prompt length for /v1/images/generations (OLMLX_IMAGE_MAX_PROMPT_CHARS).",
        ),
    ] = 8192
    ...
    # (MoE expert predictive prefetch / lookahead was removed 2026-07-11: it
    # never beat plain LRU. Root cause — expert I/O is not the bottleneck
    # (experts are page-cache-resident), so speculative reads only added
    # contention, and the routing is load-balanced so no causal predictor can
    # anticipate reuse. See docs/superpowers/specs/2026-07-11-flash-moe-*.)
